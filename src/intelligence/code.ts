...
function compileFileMatcher(pattern: string | undefined, mode: FilePatternMode): RegExp | null {
  if (!pattern) return null;
  if (mode === 'literal') return new RegExp('^' + escapeRegex(pattern) + '$', 'i');
  if (mode === 'prefix') return new RegExp('^' + escapeRegex(pattern), 'i');
  if (mode === 'glob') return globRegex(pattern);
  try {
    return new RegExp(pattern, 'i');
  } catch (error) {
    throw new Error(`filePattern is an invalid regular expression. Set filePatternMode to "literal", "prefix", or "glob" when regex syntax is not intended: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
export async function searchCode(input: {
  ...
  return await withResolvedProjectCheckout(revision, async checkout => {
    ...
    const fileMatcher = compileFileMatcher(input.filePattern, input.filePatternMode ?? 'regex');
