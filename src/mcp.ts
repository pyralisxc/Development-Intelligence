...
import { inspectEntity, projectOverview, queryWorkbench, workbenchSources } from './intelligence/workbench.js';
...
export const tools: ToolDefinition[] = [
  ...
  { name: 'investigate', description: 'Route one ordinary technical question through the same deterministic investigation contract used by the human Workbench. Returns the selected intent, resolved/ambiguous subject, chosen DI primitive, revision-bound evidence result, and no repository mutations. Specialist tools remain available for direct control.', inputSchema: objectSchema({ project: string, question: string, ref: string, graphId: string, sourceId: string, capability: technicalCapabilitySchema }, ['project', 'question']), handler: async args => await queryWorkbench({ project: s(args, 'project'), text: s(args, 'question'), ref: optString(args, 'ref'), graphId: optString(args, 'graphId'), sourceId: optString(args, 'sourceId'), capability: optString(args, 'capability') as TechnicalSourceCapability | undefined }), annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true } },
