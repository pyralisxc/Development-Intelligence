...
function boundedList(value: unknown, max = 40): unknown {
  return Array.isArray(value) && value.length > max ? value.slice(0, max) : value;
}

function compactCoverageDetail(value: any): any {
  if (!value || typeof value !== 'object') return value;
  if (Array.isArray(value)) return value.map(compactCoverageDetail);
  ...
  for (const [key, child] of Object.entries(value)) {
    ...
    output[key] = compactCoverageDetail(child);
  }
  return output;
}
...
function compactProof(value: any): any {
  if (!value || typeof value !== 'object') return value;
  return compactCoverageDetail({
    ruleId: value.ruleId,
    nodeIds: boundedList(value.nodeIds ?? []),
    edgeIds: boundedList(value.edgeIds ?? []),
    evidenceIds: boundedList(value.evidenceIds ?? []),
...
function compactRealization(value: any): any {
  ...
  return compactCoverageDetail({
    ...
    ...(Array.isArray(value.paths) ? {
      paths: value.paths.slice(0, 10),
      pathCount: value.paths.length,
      pathsTruncated: value.paths.length > 10,
    } : {}),
    ...(value.resolvedPaths ? {
      resolvedPaths: {
...
function compactFinding(value: any): any {
  ...
  return compactCoverageDetail({
    ...
    affectedIds: boundedList(value.affectedIds ?? [], 30),
...
function compactAssessment(value: any): any {
  ...
  return compactCoverageDetail({
    ...
    claims: Array.isArray(value.claims)
      ? value.claims.map((claim: any) => ({ ...claim, proof: compactProof(claim.proof) }))
      : value.claims,
    ...
    hypotheses: value.hypotheses ? {
      ...
      items: Array.isArray(value.hypotheses.items) ? value.hypotheses.items.slice(0, 10).map(compactCoverageDetail) : [],
    } : value.hypotheses,
  });
}

function compactHeadlessResult(value: any): any {
  if (!value || typeof value !== 'object') return value;
  if (Array.isArray(value)) return value.map(compactHeadlessResult);
  ...
  if ('quickNotes' in value && 'highlights' in value && 'areas' in value && 'changes' in value) {
    ...
    if (changes && typeof changes === 'object') {
      const { detail: _detail, ...summary } = changes;
      changes = { ...summary, detailTool: 'diff_graph' };
    }
    return compactCoverageDetail({
      ...value,
      changes,
  ...
  const output: Record<string, unknown> = {};
  for (const [key, child] of Object.entries(value)) {
    output[key] = key === 'result' ? compactHeadlessResult(child) : compactCoverageDetail(child);
  }
  return output;
}
...
export const tools: ToolDefinition[] = [
  ...
  { name: 'search_code', description: 'Search exact Git source at one immutable graph/revision context without requiring a persistent code index. filePattern defaults to regex matching; set filePatternMode to literal, prefix, or glob when that is the intended path-filter syntax.', inputSchema: objectSchema({ project: string, ref: string, graphId: string, pattern: string, filePattern: string, filePatternMode: filePatternModeSchema, regex: boolean, context: integer, limit: integer }, ['project', 'pattern']), handler: async args => await searchCode({ project: s(args, 'project'), ref: optString(args, 'ref'), graphId: optString(args, 'graphId'), pattern: s(args, 'pattern'), filePattern: optString(args, 'filePattern'), filePatternMode: optString(args, 'filePatternMode') as FilePatternMode | undefined, regex: args.regex === true, context: typeof args.context === 'number' ? args.context : undefined, limit: typeof args.limit === 'number' ? args.limit : undefined }) },
