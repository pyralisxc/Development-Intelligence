...
export const tools: ToolDefinition[] = [
  ...
  { name: 'refresh_codebase', description: 'Ensure an allowlisted repository ref has a validated immutable revision bundle. Set force=true to rebuild the same source revision after suspected derived-artifact damage. In managed hosting this queues a short-lived indexing job; failed indexing never replaces the selected last-known-good bundle.', inputSchema: objectSchema({ project: string, ref: string, force: boolean }, ['project']), handler: async args => await refreshCodebase(s(args, 'project'), optString(args, 'ref'), args.force === true) },
