...
import { listPublicProjects, refreshCodebase, removeDerivedProjectState } from './codebase/sourceManager.js';
...
export const tools: ToolDefinition[] = [
  { name: 'list_projects', description: 'List public project identities available to Development Intelligence. Infrastructure paths and internal Codebase Memory generation names are not exposed as project identities.', inputSchema: objectSchema({}), handler: async () => await listPublicProjects() },
  { name: 'project_status', description: 'Report independent source, Codebase Memory, and Parity Engine freshness/provenance for a project. Does not refresh either engine.', inputSchema: objectSchema({ project: string, checkUpstream: boolean }, ['project']), handler: async args => await projectStatus(s(args, 'project'), args.checkUpstream !== false) },
  { name: 'refresh_codebase', description: 'Refresh an allowlisted managed Git source and atomically promote a healthy Codebase Memory generation. Returns exact upstream/checkout/indexed revisions. Failed indexing preserves the previous selected generation.', inputSchema: objectSchema({ project: string, ref: string }, ['project']), handler: async args => await refreshCodebase(s(args, 'project'), optString(args, 'ref')) },
  { name: 'delete_project', description: 'Delete only Development Intelligence derived state for an allowlisted project. Does not modify the canonical repository.', inputSchema: projectOnly, handler: async args => await removeDerivedProjectState(s(args, 'project')) },
  { name: 'index_status', description: 'Get Codebase Memory indexing status for the currently selected generation of a public project.', inputSchema: projectOnly, handler: async args => await callCurrentCodebase(s(args, 'project'), 'index_status', {}) },
  { name: 'search_graph', description: 'Search the current Codebase Memory graph for functions, classes, routes, variables and related symbols.', inputSchema: objectSchema({ project: string, query: string, label: string, name_pattern: string, qn_pattern: string, file_pattern: string, relationship: string, min_degree: integer, max_degree: integer, exclude_entry_points: boolean, include_connected: boolean, semantic_query: strings, limit: integer, offset: integer, format: { enum: ['tree', 'json'] }, fields: strings, detail: { enum: ['ids', 'default'] } }, ['project']), handler: async args => await callCurrentCodebase(s(args, 'project'), 'search_graph', stripProject(args)) },
  { name: 'search_code', description: 'Search tracked source text and enrich matches with current Codebase Memory graph context.', inputSchema: objectSchema({ project: string, pattern: string, file_pattern: string, path_filter: string, mode: { enum: ['compact', 'full', 'files'] }, context: integer, regex: boolean, debug: boolean, limit: integer }, ['project', 'pattern']), handler: async args => await callCurrentCodebase(s(args, 'project'), 'search_code', stripProject(args)) },
  { name: 'get_code_snippet', description: 'Read source for a symbol from the selected Codebase Memory generation.', inputSchema: objectSchema({ project: string, qualified_name: string, include_neighbors: boolean }, ['project', 'qualified_name']), handler: async args => await callCurrentCodebase(s(args, 'project'), 'get_code_snippet', stripProject(args)) },
  { name: 'trace_path', description: 'Trace callers, callees, data flow, or cross-service paths through the selected Codebase Memory graph.', inputSchema: objectSchema({ project: string, function_name: string, direction: { enum: ['inbound', 'outbound', 'both'] }, depth: integer, limit: integer, cursor: string, mode: { enum: ['calls', 'data_flow', 'cross_service'] }, parameter_name: string, edge_types: strings, risk_labels: boolean, include_tests: boolean, format: { enum: ['tree', 'json'] }, include_evidence: boolean }, ['project', 'function_name']), handler: async args => await callCurrentCodebase(s(args, 'project'), 'trace_path', stripProject(args)) },
  { name: 'query_graph', description: 'Run an advanced Cypher query against the selected Codebase Memory graph or missed-file graph.', inputSchema: objectSchema({ project: string, query: string, graph: { enum: ['code', 'missed'] }, max_rows: integer }, ['project', 'query']), handler: async args => await callCurrentCodebase(s(args, 'project'), 'query_graph', stripProject(args)) },
  { name: 'get_graph_schema', description: 'Get the Codebase Memory graph schema for the selected project generation.', inputSchema: projectOnly, handler: async args => await callCurrentCodebase(s(args, 'project'), 'get_graph_schema', {}) },
  { name: 'get_architecture', description: 'Get a high-level Codebase Memory architecture overview for the selected project generation.', inputSchema: objectSchema({ project: string, path: string, aspects: strings }, ['project']), handler: async args => await callCurrentCodebase(s(args, 'project'), 'get_architecture', stripProject(args)) },
  { name: 'check_index_coverage', description: 'Check Codebase Memory coverage metadata for exact files or bounded scopes.', inputSchema: objectSchema({ project: string, paths: strings, scopes: strings, scope_limit: integer, scope_offset: integer }, ['project']), handler: async args => await callCurrentCodebase(s(args, 'project'), 'check_index_coverage', stripProject(args)) },
  { name: 'detect_changes', description: 'Map a Git diff to its Codebase Memory blast radius using the selected managed source generation.', inputSchema: objectSchema({ project: string, scope: { enum: ['files', 'impact'] }, direction: { enum: ['inbound', 'outbound', 'both'] }, depth: integer, limit: integer, base_branch: string, since: string, format: { enum: ['tree', 'json'] } }, ['project']), handler: async args => await callCurrentCodebase(s(args, 'project'), 'detect_changes', stripProject(args)) },
  { name: 'ingest_traces', description: 'Ingest runtime caller/callee traces into Codebase Memory derived state. Does not modify project source.', inputSchema: objectSchema({ project: string, traces: { type: 'array', items: objectSchema({ caller: string, callee: string, count: integer }) } }, ['project', 'traces']), handler: async args => await callCurrentCodebase(s(args, 'project'), 'ingest_traces', stripProject(args)) },
  { name: 'scan_parity', description: 'Scan available technical representations using generic technology analyzers only. No project-specific extractor or semantic configuration is used. Runtime URLs must belong to allowlisted operational origins and are observed read-only.', inputSchema: objectSchema({ project: string, urls: { type: 'array', items: { type: 'string', format: 'uri' }, maxItems: 20 } }, ['project']), handler: async args => { const scan = await scanParity(s(args, 'project'), Array.isArray(args.urls) ? args.urls.filter(value => typeof value === 'string') as string[] : []); return { project: scan.project, scanId: scan.scanId, createdAt: scan.createdAt, repositoryRevision: scan.repositoryRevision, sources: scan.sources, observationCount: scan.observations.length, resolutionCounts: scan.resolutions.reduce((acc, item) => ({ ...acc, [item.status]: (acc[item.status] ?? 0) + 1 }), {} as Record<string, number>), namingDivergenceCount: scan.namingDivergences.length, unmatchedObservationCount: scan.unmatchedObservationIds.length, unavailableSourceIds: scan.unavailableSourceIds }; } },
