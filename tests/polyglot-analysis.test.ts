...
test('C# repository binding resolves interfaces, constructors, and syntactically safe cross-file calls without guessing instance dispatch', async () => {
...
`);
    ...
    assert.equal(graph.edges.some(edge => edge.from === execute.id && edge.to === run.id && edge.kind === 'calls' && edge.status === 'resolved'), false, 'instance dispatch through a local variable must not be guessed without type evidence');
  } finally {
    await fs.rm(root, { recursive: true, force: true });
  }
});
