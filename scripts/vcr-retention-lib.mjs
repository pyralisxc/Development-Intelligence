...
export function planVcrRetention({ images, deployments, now = Date.now(), limit = 50, nonProductionDays = 1, productionDays = 7, rollbackCount = 1 }) {
  const readyProduction = deployments
    .filter(item => isProduction(item) && ['READY', 'ready'].includes(item.state ?? item.readyState))
  ...
  const decisions = images.map(image => {
    ...
    const protectsDeployment = matches.some(record => protectedDeploymentIds.has(record.id));
    ...
    else if (protectsDeployment) reason = 'active production or rollback deployment';
    ...
    return { id, digest: image.digest ?? null, tags, createdAt: createdAt || null, ageDays: ageDays === null ? null : Number(ageDays.toFixed(2)), action, reason };
  });

  const deleteCount = decisions.filter(item => item.action === 'delete').length;
  return {
    policy: { limit, nonProductionDays, productionDays, rollbackCount },
    counts: {
